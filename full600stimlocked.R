...
library(rlist)
library(ggplot2)
data <- read.csv('stimlockedmyData.csv')
...
a = ggplot(age_binned, aes(X, A)) +
  ...
  ggtitle("stim locked Fixed effect age") +
...
c = ggplot(cond_binned, aes(X, A)) +
  ...
  ggtitle("stim locked Fixed effect cond (pro/anti)") +
...
i = ggplot(inter_binned, aes(X, A)) +
  ...
  ggtitle("stim locked Interaction age*cond") +
  theme_light()
